...
function planets.load_tick(tick) -- I wish this didn't need to be on_tick
	if (tick > 0) then
		if (storage.migrated) then
			planets.post_dependencies_loaded()
...
function planets.post_dependencies_loaded()
	events.vraise("on_started", { templates = storage.templates })
end
...
function planets.call(world, evname, ev)
	if (world[evname]) then
		if (remote.interfaces[world[evname][1]]) then
			return
				remote.call(world[evname][1], world[evname][2], ev, world)
		end
	end
end
...
function planets.GetBySurface(f)
	local ind = istable(f) and f.index
	for i in pairs(game.surfaces) do if (i.index == f) then ind = ind or i.index end end
	--	ind=ind or game.surfaces[f].index
	return storage.worlds[isnumber(f) and f or ind] or nil
end

function planets.DestroyWorld(world, ev)
	planets.call(world, "on_pre_surface_destroyed", ev)
	storage.worlds[world.surface.index] = nil
end

function planets.OnPreSurfaceDestroyed(ev)
	local world = planets.GetBySurface(ev.surface_index)
	if (world) then planets.DestroyWorld(world, ev) end
end

events.hook(defines.events.on_pre_surface_deleted, planets.OnPreSurfaceDestroyed)

function planets.OnSurfaceCreated(ev)
	local world = planets.GetBySurface(events.surface(ev))
	if (world) then planets.call(world, "on_surface_created") end
end

events.hook("on_surface_created", planets.OnSurfaceCreated)

function planets.OnPreSurfaceCleared(ev)
	local world = planets.GetBySurface(events.surface(ev))
	if (world) then planets.call(world, "on_pre_surface_cleared") end
end

events.hook("on_pre_surface_cleared", planets.OnSurfaceCreated)

function planets.OnChunkGenerated(ev)
	local world = planets.GetBySurface(events.surface(ev))
	if (world) then
		planets.call(world, "on_chunk_generated", ev)
		local fc = world.force
		if (fc) then
			for k, v in pairs(fc.surface.find_entities_filtered { force = game.forces.enemy, area = ev.area }) do
				v.force =
					fc
			end
		end
...
function planets.OnBiterBase(ev)
	local e = ev.entity
	if (isvalid(e)) then
		local world = planets.GetBySurface(e.surface)
		if (world) then
...
function planets.RegisterDefaultTemplate(tbl) -- internal constructor called during script include
	tbl.mod = "planetorio"
	planets.templates[tbl.key] = tbl
end

function planets.ValidateTemplates() -- check templates in global table against installed mods
	local tbr = {}
	for k, v in pairs(storage.templates) do if (v.mod and not script.active_mods[v.mod]) then tbr[k] = true end end
...
function planets.RegisterTemplate(tbl) -- register template to global table.
	storage.templates[tbl.key] = table.deepcopy(tbl)
	events.vraise("on_new_template", { template = tbl })
end
...
function planets.RemoveTemplate(key) -- Remove a template. Particularly needed for migrating planet mods that have removed templates from themselves
	local tmp = storage.templates[key]
	if (not tmp) then return false end
	events.vraise("on_template_removed", { template = tmp })
	storage.templates[key] = nil
end

function planets.UpdateTemplate(tbl) -- update a template in the global table
	if (not storage.templates[tbl.key]) then
		error("Tried to update a template that doesnt exist! : " ..
			tostring(tbl.key))
	end
	table.deepmerge(planets.templates[tbl.key], tbl)
	events.vraise("on_template_updated", { template = tbl })
end

function planets.GetTemplate(key) return storage.templates[key] end

function planets.GetTemplates() return storage.templates end
...
function planets.Modify(g, mdt, ...)
	...
	if (not mdf) then return end
	if (mdf.gen) then planets.Merge(g, mdf.gen) end
	if (mdf.fgen) then mdf.fgen(g, mdt[2], ...) end
	if (mdf.fgen_call) then
		local gx, dx = planets.call(mdf.fgen_call, g, mdt[2], ...) or {}
		if (gx) then planets.Merge(g, gx) end
		if (dx) then planets.Merge(datas, dx) end
	end
	return datas
end
...
function planets.Generate(planet, gen, ...)
	...
	if (planet.modifiers) then
		for id, mdt in ipairs(planet.modifiers) do
			local x = planets.Modify(gen, mdt, ...)
			if (x) then planets.Merge(datas, x) end
		end
	end
	if (planet.biomes) then
		for id, tmp in ipairs(planet.biomes) do
			local temp = storage.templates[tmp]
			if (temp) then
				local mdr = temp.modifiers
				if (mdr) then
					for i, mdt in ipairs(mdr) do
						if (mdt[1] ~= "nauvis") then
							local x = planets.Modify(gen, mdt, ...)
							if (x) then planets.Merge(datas, x) end
						end
					end
	...
	if (planet.fgen) then
		local gx, dx = planet.fgen(gen, planet, ...)
		if (gx) then planets.Merge(g, gx) end
		if (dx) then planets.Merge(datas, dx) end
	end
	if (planet.fgen_call) then
		local gx, dx = planets.call(planet.fgen, gen, planet, ...)
		if (gx) then planets.Merge(g, gx) end
		if (dx) then planets.Merge(datas, dx) end
	end
	return gen, datas
...
function planets.MakeSurface(name, planet, gen, ...)
	local f = game.surfaces[name]
	if (f) then f.map_gen_settings = gen else f = game.create_surface(name, gen) end
	planet.surface = f
	storage.worlds[f.index] = planet
	storage.worlds[f.index].surface = f

	if (planet.mod_table) then remote.call(planet.mod, "INSERT_MOD_TABLE", f) end

	if (planet.modifiers) then
		for i, mdt in ipairs(planet.modifiers) do
			local mdf = planets.Modifiers[mdt.name or mdt[1]]
	...
	if (not planet.no_first_chunks) then
		f.request_to_generate_chunks({ 0, 0 }, 5)
...
function planets.FromTemplate(name, key)
	local tmp = table.deepcopy(storage.templates[key])
	if (not tmp) then
		game.print("PLANET ERROR: FromTemplate() - planet does not exist: " ..
			tostring(key) .. ", defaulting to uncharted")
		tmp = table.deepcopy(storage.templates["uncharted"])
	end
...
function planets.EvaluateRoll(planet, opts)
	local prev_planet, flags = opts.prevplanet, opts.flags
	if (planet.required_controls) then for k, v in pairs(planet.required_controls) do if (not game.default_map_gen_settings.autoplace_controls[v]) then return false end end end
...
function planets.SimpleTemplateRoll(opts)
	opts = opts or {}
	...
	local temps = {}
	local rnjesus = 0
	for key, template in pairs(storage.templates) do
		if (not template.biome and template.zone < zone and template.rng > 0 and planets.EvaluateRoll(template, opts)) then
			rnjesus = rnjesus + (template.rng or 1)
			temps[key] = template
		end
	end
	if (rnjesus == 0) then return storage.templates["normal"] end

	local rng = math.random(1, rnjesus)
	for key, template in pairs(temps) do
		rng = rng - (template.rng or 1)
		if (rng <= 0) then return template end
	end
	return storage.templates["normal"]
end

function planets.SimplePlanetRoll(name, opts)
	opts = opts or {}
...
planets.remote.GetBySurface = planets
	.GetBySurface                                  -- Returns World Table, which is a planet template table with .surface=f
...
-- /sc local f=remote.call("planet","FromTemplate","test1","ocean") game.player.teleport({0,0},test1)
